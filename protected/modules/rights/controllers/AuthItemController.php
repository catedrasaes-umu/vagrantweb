...
class AuthItemController extends RController
{
	...
	public function actionCreate()
	{
		...
		if( isset($_POST['AuthItemForm'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
				...
				{					
					$this->redirect(array('authItem/update', 'name'=>urlencode($formModel->name)));
				}else{
	...
	*/
	public function actionUpdate()
	{
		...
		//if (($itemName=="AdminRole"))		
		if ($this->_authorizer->isAdminRole($itemName))
		{
			Yii::app()->user->setFlash($this->module->flashErrorKey,
				"This Role can't be updated"
			);
		...
		$parentSelectOptions = Rights::getParentAuthItemSelectOptions($model, CAuthItem::TYPE_ROLE, $exclude);
		
		

		if( $parentSelectOptions!==array() )
		{


			$parentFormModel = new AuthParentForm();
		
			// Child form is submitted and data is valid
			if( isset($_POST['AuthParentForm'])===true )
			{

				$parentFormModel->attributes = $_POST['AuthParentForm'];
				
				if( $parentFormModel->validate()===true )
				{
					// Add the parent and load it
					$this->_authorizer->authManager->addItemChild($itemName, $parentFormModel->itemname);
					$parent = $this->_authorizer->authManager->getAuthItem($parentFormModel->itemname);
					$parent = $this->_authorizer->attachAuthItemBehavior($parent);

					// Set a flash message for adding the parent
					Yii::app()->user->setFlash($this->module->flashSuccessKey,
						Rights::t('core', 'Parent :name added.', array(':name'=>$parent->getNameText()))
					);
		...
		$childSelectOptions = Rights::getParentAuthItemSelectOptions($model, CAuthItem::TYPE_ROLE, $exclude);
		...
		if( $childSelectOptions!==array() )
		{
			...
			if( isset($_POST['AuthChildForm'])===true )
			{
				...
				if( $childFormModel->validate()===true )
				{
					...
					$this->_authorizer->authManager->addItemChild($childFormModel->itemname,$itemName );
					$child = $this->_authorizer->authManager->getAuthItem($itemName);
					$child = $this->_authorizer->attachAuthItemBehavior($childFormModel->itemname);

					// Set a flash message for adding the child
					Yii::app()->user->setFlash($this->module->flashSuccessKey,
						Rights::t('core', 'Child :name added.', array(':name'=>$childFormModel->itemname))
					);
		...
		$operationSelectOptions = Rights::getParentAuthItemSelectOptions($model, $type, $exclude);
		unset($operationSelectOptions['Roles']);
		

		if( $operationSelectOptions!==array() )
		{
			$operationFormModel = new AuthChildForm();
		
			// Child form is submitted and data is valid
			if( isset($_POST['AuthChildForm'])===true )
			{
				$operationFormModel->attributes = $_POST['AuthChildForm'];
				if( $operationFormModel->validate()===true )
				{
					// Add the child and load it
					$this->_authorizer->authManager->addItemChild($itemName, $operationFormModel->itemname);
					$child = $this->_authorizer->authManager->getAuthItem($operationFormModel->itemname);
					$child = $this->_authorizer->attachAuthItemBehavior($child);
		...
		$parentDataProvider = new RAuthItemChildDataProvider($model);
		$parentDataProvider->type=CAuthItem::TYPE_ROLE;

		$childDataProvider = new RAuthItemParentDataProvider($model);
		$childDataProvider->type=CAuthItem::TYPE_ROLE;

		
		$operationDataProvider = new RAuthItemChildDataProvider($model);
		$operationDataProvider->type=CAuthItem::TYPE_OPERATION;

		$taskDataProvider = new RAuthItemChildDataProvider($model);
		$taskDataProvider->type=CAuthItem::TYPE_TASK;

		$operationDataProvider->setData(array_merge($operationDataProvider->data,$taskDataProvider->data));



		// $parentOperationDataProvider = new RAuthItemParentDataProvider($model);
		// $parentOperationDataProvider->type=CAuthItem::TYPE_OPERATION;

		

		//$parentOperationDataProvider = new RAuthItemDataProvider($model);
		//$parentOperationDataProvider->type=CAuthItem::TYPE_OPERATION;

		$inheritedProvider = new RPermissionDataProvider('permissions');
		
		
		//var_dump($inheritedProvider->generatePermsInheritedforRole($model));





		$parentOperationDataProvider=new CArrayDataProvider($inheritedProvider->generatePermsInheritedforRole($model),
															array(
															    'id'=>'id',
															    'sort'=>array(
															        'attributes'=>array(
		...
		// var_dump($parentOperationDataProvider->totalItemCount);
		// var_dump($parentOperationDataProvider->getData());exit;

		//var_dump($parentOperationDataProvider->getData());exit;

		// var_dump(Rights::getAuthorizer()->getPermissions('DEMOROLE'));
		// var_dump(Rights::getAuthorizer()->getPermissions()['DEMOROLE']);exit;

		// var_dump($parentOperationDataProvider);exit;
		//var_dump($ndataProvider->getData());exit;
		...
		var_dump($ndataProvider->getData());exit;
		var_dump($model);exit;
		var_dump(Rights::getAssignedRoles($model->id));exit;*/
	...
	public function actionDelete()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			//if (($itemName=="AdminRole") && ($item->type==2))
			if ($this->_authorizer->isAdminRole($itemName) && ($item->type==2))
			{
				Yii::app()->user->setFlash($this->module->flashErrorKey,
	...
	*/
	public function actionRemoveChild()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			Yii::app()->user->setFlash($this->module->flashSuccessKey,
				Rights::t('core', 'Item :name removed.', array(':name'=>$child->getNameText()))
			);
			...
			if( isset($_POST['ajax'])===false ){
				...
				if (!isset($_GET['redirect']))
					...
					$this->redirect(array('authItem/update', 'name'=>urlencode($childName)));
			}
