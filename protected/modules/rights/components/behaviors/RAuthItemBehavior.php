...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	...
	{		
		return ($this->owner->description!==null) ? $this->owner->description : $this->owner->name;
	}


	public function getDescriptionLink()
	{
		return CHtml::link($this->getDescriptionText(), array(
			'authItem/update',
	...
	*/
	...
	public function getGridNameLinkSimple()
	{
		$markup = CHtml::link($this->owner->name, array(
			'authItem/update',
			'name'=>urlencode($this->owner->name),
	...
	*/
	...
	public function getRemoveParentLink2()
	{
		return CHtml::linkButton(Rights::t('core', 'Remove'), array(
			'submit'=>array('authItem/removeChild', 'name'=>urlencode($this->owner->name), 'redirect'=>'false','child'=>urlencode($this->parent->name)),
			'class'=>'remove-link',
			'csrf'=>Yii::app()->request->enableCsrfValidation,
		));
	}
