...
class CURL extends CComponent
{
	...
	public function execute()
	{
		...
		if ( ! isset($this->options[CURLOPT_FAILONERROR]))
		{
			...
			#fjsanpedro@gmail.com : Adding option			
		...
		else
		{
			...
			//Modified fjsanpedro@gmail.com 13/09/2013
			/////////////////////////////////////////////
			if ( isset($this->options[CURLOPT_HEADER]) && 
				($this->options[CURLOPT_HEADER]===TRUE ||$this->options[CURLOPT_HEADER]===1) )
			{
										
				list($my_header, $my_body) = explode("\r\n\r\n", $this->response, 2);
				//debug($my_header);
				
				
				//http://www.php.net/manual/en/function.http-parse-headers.php#77241
				$curl_headers = explode("\r\n", preg_replace('/\x0D\x0A[\x09\x20]+/', ' ', $my_header));
				
				foreach( $curl_headers as $cheader ) {
		            if( preg_match('/([^:]+): (.+)/m', $cheader, $match) ) {
		                $match[1] = preg_replace('/(?<=^|[\x09\x20\x2D])./e', 'strtoupper("\0")', strtolower(trim($match[1])));
		                if( isset($retVal[$match[1]]) ) {
		                    $response_headers[$match[1]] = array($response_headers[$match[1]], $match[2]);
		                } else {
		                    $response_headers[$match[1]] = trim($match[2]);
		                }
		            }
	        	}
